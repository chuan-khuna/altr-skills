 ...
 *   2. BASELINE          — text element tops, modulo the baseline unit, ~0
 *      (measured per-spread, relative to that spread's `.grid` top).
 *   3. OPTICAL INK       — display elements' visible INK-left equals the column
...
(async () => {
  ...
  for (const W of widths) {
    ...
    const res = await page.evaluate((BL) => {
      ...
      // 2. baseline (tops modulo BL, per spread relative to its .grid top)
      let baseErr = 0;
      document.querySelectorAll('.spread').forEach(sp => {
        const gridEl = sp.querySelector('.grid'); if (!gridEl) return;
      ...
      // 3. optical ink offset — each display element's visible INK-left must
    ...
    const pass = res.maxColErrPx <= 0.5 && res.maxBaselineOffPx <= (BL / 2) && res.maxInkOffPx <= 1.0;
    if (!pass) failed = true;
    console.log(`[${pass ? 'PASS' : 'FAIL'}] vw=${W}  col=${res.maxColErrPx}px ` +
